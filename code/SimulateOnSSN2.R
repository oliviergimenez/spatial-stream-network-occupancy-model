SimulateOnSSN2 <- function(ssn.object,
		ObsSimDF,
		PredSimDF = NULL,
		PredID = NULL,
		formula,
	...
	)
{
  ...
  ocoord <- ssn.object@obspoints@SSNPoints[[1]]@point.coords
	...
	if(family == "Binomial" | family == "binomial") {
		...
		{
	    # OG modification Jan 2024
		  prob[,1] <- exp(svo[,1])/(1 + exp(svo[,1]))
	#	  svo[,1] <- rbinom(length(svo[,1]),1,prob[,1])
	  svo[,1] <- rbinom(length(svo[,1]),1,exp(svo[,1])/(1 + exp(svo[,1])))}}
